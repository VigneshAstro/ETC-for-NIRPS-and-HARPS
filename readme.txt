...
1. First we need to install ExoFile from Antoine Darveau's GitHub: https://github.com/AntoineDarveau/exofile
...
(A). Are the input parameters filled? (Y/N): 

If you have already filled the input parameter file (input_file.py) enter Y here.
If not enter N.

(B). Please enter the planet's name:

Please enter the planet name in the standard form:  example WASP-121 b (or) TRAPPIST-1 b (or) HD 209548 b.

(C). Please enter NIRPS mode HA/HE (eg: he): 
(D). Please enter HARPS mode ham/eggs (eg: ham): 

Please enter which mode you want to observe in HARPS and NIRPS.

(E). Please enter airmass (between 1 - 2.9): 1.2
(F). Please enter precipitable water vapour in mm (between 0.05 - 30): 2.5

Please enter the atmospheric conditions as requested.

After entering this, the code uses fill_param.py to fill the input_file.py
