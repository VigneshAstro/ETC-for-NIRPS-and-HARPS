...
def ks_kp(M_s,M_p,a,ecc,inc):
    ...
    def K_p(k_star,m_star,m_planet):
        return np.float64(k_star)*(np.float64(m_star)+np.float64(m_planet))/(np.float64(m_planet))
...
def time_to_phase(time,time_ref,period):
    phase = ((time - time_ref) % period) / period
