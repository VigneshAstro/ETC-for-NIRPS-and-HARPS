...
for tim in exp:
    harps550 = etc_estimate(exp, tim, mag_band[0], Vmag)
    y = etc_estimate(exp, tim, mag_band[1], Ymag)
    #J = etc_estimate(exp, tim, mag_band[2], Jmag)
    H = etc_estimate(exp, tim, mag_band[3], Hmag)
...
if calc_pb.upper() == "Y":
    ...
    PB, comb_SNR_y, comb_SNR_H, comb_SNR_harps550 = blur(exp, y_list, H_list, harps550_list) # PB in km/s
else:
...
if PB is not None:
    ...
    np.savetxt(os.path.join(output_folder, f'{name}.txt'), np.column_stack((exp, harps550_list, y_list, H_list, PB, comb_SNR_y, comb_SNR_H, comb_SNR_harps550)), fmt='%1.8f', header='Exp. Time (s)\tharps550\ty\tH\tPB (km/s/pixel)\tSNR_y_tran\tSNR_H_tran\tSNR_harps550_tran')
    ...
    np.savetxt(os.path.join(output_folder, f'{name}.txt'), np.column_stack((exp, harps550_list, y_list, H_list)), fmt='%1.8f', header='Exp. Time (s)\tharps550\ty\tH')
